...
function speakTextOnCard(event) {
    ...
    let text = "";
    let lang = BASE_LANG.dialect;
    if (hint.style.display == "block") text = hint.innerText;
    else text = answer.innerText;
...
function loadPage(serverResponse) {
    ...
    generateModeButtons();
    generateListsDropdown(VERBS_PAGE, dropdownSelectCallbackIndexPage, actionButtonCallbackIndexPage);
    createAllList();
    //getSaved();
    //updateSaved();
    updateModeView();
...
function start() {
    ...
    cardIdx = 0;
    showCard();
    let title = (list == "all" ? "All Verbs" : (list == "saved") ? "Saved" : vocabLists[list].title);
